...
const availableImages = [
  ...
  "images/aficandog.webp","images/armor.webp","images/badger.webp",
  "images/capybara.webp","images/cheeta.webp","images/dragon.webp",
  "images/fenic.webp","images/hyena.webp","images/maned.webp",
  "images/ocelot.webp","images/otter.webp",
];
...
function togglePause() {
  if (isPaused) {
    ...
    isPaused = false;
    boardLocked = false;
    startTimer(true); // pass "resume" flag
...
function setupBoard() {
  ...
  // Stop any previous timer and clear display
  ...
  // Ensure container is visible so we see the slide-in
  gameContainer.style.transition = gameContainer.style.transition || "opacity .2s ease";
  gameContainer.style.opacity = 1;

  // Reset layout class
  gameBoard.className = "";

  // Apply fixed 8x4 layout when totalCards is 32
  if (totalCards === 32) {
    gameBoard.classList.add("board-32");
  } else {
    applyBoardDensity(); // fallback to existing logic for smaller boards
  }
  ...
  const delayStep = 0.05; // seconds between card animations

  // Build all cards into a fragment to avoid intermediate paints
  const frag = document.createDocumentFragment();
  const inners = []; // keep refs for animation setup + completion tracking
  ...
  card.innerHTML = `
    ...
    // Start each card pre-hidden & translated so first paint isn't final position
    inner.style.opacity = "0";
    inner.style.transform = "translateY(50px)";
    inner.style.animationDelay = `${i * delayStep}s`;

    inners.push(inner);
    frag.appendChild(card);
  }

  // Append all at once (no visible flash)
  gameBoard.appendChild(frag);

  // Fill backs after DOM exists
  printBlanks();

  // Next frame: trigger animations by adding the class and clearing inline start styles
  requestAnimationFrame(() => {
    inners.forEach(inner => void inner.offsetWidth);
  ...
  // Wait for all slide-ins to finish before unlocking + starting the timer
  if (inners.length === 0) {
    boardLocked = false;
    startTimer();
    if (typeof showStartMessage === "function") showStartMessage();
    return;
  }
...
function nextLevel() {
  ...
  if (totalCards < MAX_CARDS) {
    totalCards = Math.min(totalCards + 2, MAX_CARDS);
  }
...
function startTimer(resume = false) {
  ...
  if (!resume) {
    const base = baseTime(level);
    timeLeft = base + (bankedTime || 0);
    bankedTime = 0;
  }
...
if (loseStats) {
  ...
  loseStats.textContent = `You reached Level ${level} of 20 and matched ${totalMatchedAllLevels} pairs in total.`;}
